...
Node roles. The **local node** (the one this server is connected to) is **always a companion** (\`companion_radio_wifi\` / \`companion_radio_usb\` firmware) — a controller-facing gateway. Other nodes on the mesh may be **repeaters** (routing/flood control), **rooms** (chat servers), or other companions. The split matters for \`admin\`: most enumerated admin commands wrap **repeater-firmware CLI verbs** the companion doesn't implement, so they're \`remote-only\` and must target a repeater by name as a remote contact. Companion-local state (radio basics, identity, channels, contacts) is reachable via structured tools (\`set_channel\`, \`import_contact\`, …) and the few \`admin\` commands with a home path (\`reboot\`, \`advert\`, \`sync-time\`, \`set-tx-power\`, \`set-radio\`, \`set-name\`, \`set-location\`). Telemetry-config on the local companion is **not** exposed by the companion protocol at all — that's a firmware-only feature; configure it via serial.
...
- **Unwrapped admin shortcuts:** ${unwrappedNames} — top-level forms of selected admin commands so they carry per-command MCP annotations (e.g. \`set_tx_power\` is idempotent, \`get_node_neighbors\` is read-only, \`reboot_node\` is destructive) and typed input schemas. Two flavours: the *home+remote* set (\`reboot_node\` / \`broadcast_advert\` / etc.) takes an optional \`node\` (omit ⇒ home); the *remote-only* set (\`get_node_*\` reads + \`discover_neighbors\`) requires \`node\` because those CLI verbs exist only on repeater firmware. Both forms reach the same dispatch as the multiplexed \`admin\` tool; pick whichever your client tooling prefers. For the home node's firmware/board/clock/auto-add state, \`get_node_health()\` returns them as structured fields — no admin call needed.
